# Presentation Builder# Presentation Builder# Presentation Builder

**Автоматизация создания PowerPoint презентаций из JSON конфигураций**Автоматизация создания PowerPoint презентаций из JSON конфигураций.**Автоматизация создания PowerPoint презентаций из структурированных данных**

[![Python 3.7+](https://img.shields.io/badge/python-3.7+-blue.svg)](https://www.python.org/downloads/)## Быстрый старт[![Python 3.7+](https://img.shields.io/badge/python-3.7+-blue.svg)](https://www.python.org/downloads/)
...
Модульная система для автоматической генерации PowerPoint презентаций из структурированных JSON конфигураций. Устраняет ручной труд по копированию-вставке заголовков, заметок и изображений.
...
pip install -r requirements.txt- [Быстрый старт](#-быстрый-старт)

```

`config.json`:- [Возможности](#-возможности)

### 2. Создайте конфигурацию

```json- [Архитектура](#-архитектура)

`config.json`:

```json{- [Документация](#-документация)

{

  "template_path": "template.pptx",  "template_path": "template.pptx",- [Разработка](#-разработка)

  "layout_name": "VideoLayout",

  "output_path": "output.pptx",  "layout_name": "VideoLayout",
...
```      "title": "Слайд 1",**Presentation Builder** — это модульная система для автоматической генерации PowerPoint презентаций из JSON конфигураций.



### 3. Сгенерируйте презентацию      "notes_source": "notes.md",



```bash      "images": ["photo.jpg"]### Основная задача
...
Готово! Результат в `output.pptx` 🎉  ]Устранение ручного труда по копированию-вставке:
...
## 📋 CLI команды```- Заголовков и номеров слайдов
...
python main.py analyze --template template.pptx [--layout "LayoutName"]

```bash### Для кого это

# Справка

python main.py helppython main.py generate --config config.json
...
## CLI команды- **Бизнес**: генерация отчетов и аналитики
...
| `template_path` | string | Да | Путь к шаблону PPTX |

| `layout_name` | string | Да | Имя макета из шаблона |# Генерация презентации---

| `output_path` | string | Нет | Путь к выходному файлу (по умолчанию: `output.pptx`) |

| `slides` | array | Да | Массив слайдов |python main.py generate --config slides.json [--output result.pptx] [--verbose]
...
| Поле | Тип | Обязательно | Описание |# Анализ шаблона

|------|-----|-------------|----------|

| `layout_type` | string | Да | Тип размещения: `single_wide`, `single_tall`, `two_stack`, `two_tall_row` |python main.py analyze --template template.pptx [--layout "LayoutName"]### Установка

| `title` | string | Да | Заголовок слайда |

| `notes_source` | string | Да | Путь к .md файлу ИЛИ inline текст |

| `images` | array | Да | Пути к изображениям (относительные или абсолютные) |
...
---### Обязательные поляpip install -r requirements.txt
...
См. [`doc/samples/`](doc/samples/) для готовых примеров:- `template_path` — путь к PPTX шаблону



- **simple_example.json** — базовый пример с одним изображением- `layout_name` — имя макета из шаблона (узнать: `analyze`)### Первая презентация

- **multi_image_example.json** — пример с двумя изображениями

- **absolute_paths_example.json** — демонстрация абсолютных путей- `slides[]` — массив слайдов
...
Пишите заметки в Markdown — система автоматически очистит форматирование:- `layout_type` — тип макета (single_wide, single_tall, two_stack, two_tall_row){



```markdown- `title` — заголовок слайда  "template_path": "template.pptx",
...
- **single_tall** — 1 высокое изображение (9:16)      "notes_source": "notes.md",

```json

{- **two_stack** — 2 изображения вертикально      "images": ["photo.jpg"]

  "template_path": "C:/Templates/corporate.pptx",  // абсолютный

  "slides": [- **two_tall_row** — 2 высоких изображения рядом    }
...
├── core/                # Бизнес-логика (builder, cleaner, analyzer)```
...
├── tests/               # Unit тесты с pytest3. **Откройте результат**: `result.pptx` 🎉

└── doc/                 # Документация и примеры

    ├── samples/         # Примеры конфигураций```bash

    └── REFERENCE.md     # Полная справка

```pytest tests/### Примеры
...
- **SRP**: каждый модуль — одна ответственностьСм. готовые примеры в [`doc/samples/`](samples/):

- **DRY**: переиспользуемые компоненты

- **YAGNI**: только необходимая функциональность## Структура проекта
...
├── main.py              # CLI точка входа- `absolute_paths_example.json` — абсолютные пути

```bash

# Запуск всех тестов├── config/              # Настройки и макеты
...
pytest tests/ -v├── models/              # Модели данных
