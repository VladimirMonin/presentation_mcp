...
Автоматизация создания PowerPoint презентаций из JSON конфигураций.**Автоматизация создания PowerPoint презентаций из структурированных данных**



## Быстрый старт[![Python 3.7+](https://img.shields.io/badge/python-3.7+-blue.svg)](https://www.python.org/downloads/)
...
{- [Документация](#-документация)
  ...
  "template_path": "template.pptx",- [Разработка](#-разработка)
  ...
  "slides": [
    ...
    {## 🎯 Обзор
      ...
      "images": ["photo.jpg"]### Основная задача
...
### 3. Генерация- Изображений с автоматическим масштабированием
...
python main.py generate --config slides.json [--output result.pptx] [--verbose]

## 🚀 Быстрый старт

# Анализ шаблона

python main.py analyze --template template.pptx [--layout "LayoutName"]### Установка
...
- `layout_name` — имя макета из шаблона (узнать: `analyze`)### Первая презентация
...
- `layout_type` — тип макета (single_wide, single_tall, two_stack, two_tall_row){

- `title` — заголовок слайда  "template_path": "template.pptx",

- `notes_source` — путь к .md файлу ИЛИ inline текст  "layout_name": "single_wide",

- `images[]` — массив путей к изображениям  "output_path": "result.pptx",
...
- **two_tall_row** — 2 высоких изображения рядом    }
...
- **REFERENCE.md** — полная справка по всем возможностямpython main.py generate --config my_presentation.json
...
├── io_handlers/         # Работа с файлами## ✨ Возможности
